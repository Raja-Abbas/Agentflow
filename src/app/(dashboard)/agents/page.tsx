...
import type { Agent } from "@/lib/utils";
...
function modelColor(model: string) {
  switch (model) {
    case "GPT-4": return "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400";
    case "Claude": return "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400";
...
export default function AgentsPage() {
  ...
  useEffect(() => {
    if (agentList.length > 0) localStorage.setItem("af_agents", JSON.stringify(agentList));
  }, [agentList]);

  function openCreate() {
    setEditingAgent(null);
    setForm({ name: "", description: "", model: "GPT-4", status: "Active" });
    setDialogOpen(true);
  }
  ...
  function saveAgent() {
    ...
    if (editingAgent) {
      setAgentList((prev) =>
        prev.map((a) => a.id === editingAgent.id ? { ...a, ...form, conversations: a.conversations } : a)
      );
    } else {
  ...
  function deleteAgent(id: number) {
    if (confirm("Delete this agent?")) setAgentList((prev) => prev.filter((a) => a.id !== id));
  }
  ...
  const filterCounts = filters.map((f) => ({
    label: f,
    count: f === "All" ? agentList.length : agentList.filter((a) => a.status === f).length,
  }));
      ...
      {filteredAgents.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-slate-500">Try adjusting your search or filter, or create a new agent.</p>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={agent.id} className="transition-shadow hover:shadow-md dark:hover:shadow-slate-800/50">
              ...
              <CardContent>
                <div className="flex flex-wrap items-center gap-2 mb-4">
                  <Badge variant={statusVariant(agent.status) as "default" | "secondary" | "destructive" | "outline"}>{agent.status}</Badge>
                ...
                <div className="flex items-center gap-2 border-t border-slate-200 pt-4 dark:border-slate-700">
                  <Button variant="ghost" size="icon" title="Edit" onClick={() => openEdit(agent)}>
                    <Pencil className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="icon" title="Chat" onClick={() => window.location.href = `/conversations?agent=${encodeURIComponent(agent.name)}`}>
                    <MessageSquare className="h-4 w-4" />
                  </Button>
                  <Button variant="ghost" size="icon" title="Delete" className="text-red-500 hover:text-red-600" onClick={() => deleteAgent(agent.id)}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="space-y-2">
              ...
              <Input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="What does this agent do?" />
            </div>
            <div className="space-y-2">
              <Label>Model</Label>
              <Select
          ...
          <DialogFooter>
            ...
            <Button onClick={saveAgent}>{editingAgent ? "Save" : "Create"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
